...
async fn main() -> anyhow::Result<()> {
    ...
    env_logger::init();
    test_immediate().await?;
    test_delayed().await?;

    async fn test_immediate() -> anyhow::Result<()> {
        let auto_commit = AutoCommit::Immediate;
    ...
    async fn test_delayed() -> anyhow::Result<()> {
        let auto_commit = AutoCommit::Delayed;
        println!("AutoCommit = {auto_commit:?} ...");
        ...
        // now end the consumer, let it commit
        full.end().await?;
